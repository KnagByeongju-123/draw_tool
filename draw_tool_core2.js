...
function renderPenShapeFields(type, ex){
  ...
  if (type === 'arc'){
    ...
    if (ex){
      ...
      x = Math.min(m1.x, m2.x); y = Math.max(m1.y, m2.y);  // 좌상단 (Y up)
      w = Math.abs(m2.x - m1.x); h = Math.abs(m2.y - m1.y);
    } else if (penOriginUserCoord){
      x = penOriginUserCoord.x; y = penOriginUserCoord.y;
    }
    div.innerHTML = `
      <label>좌상단 X (mm) <input id="psf-x" type="text" value="${x.toFixed(2)}"></label>
      <label>좌상단 Y (mm) <input id="psf-y" type="text" value="${y.toFixed(2)}"></label>
    ...
    hint.textContent = 'X,Y = 좌상단 모서리 좌표 · W=가로, H=세로 (Y는 위쪽 양수)';
...
function applyPenShape(){
  ...
  if (type === 'arc'){
    ...
    const x = _parseNum('psf-x'), y = _parseNum('psf-y');
    const w = _parseNum('psf-w'), h = _parseNum('psf-h');
    if (w <= 0 || h <= 0){ alert('너비/높이는 0보다 커야 합니다'); return; }
    const p1 = penUserMmToPx(x, y);
    const p2 = penUserMmToPx(x + w, y - h);  // Y up = 화면 아래로
