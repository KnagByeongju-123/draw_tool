...
function penDrawPreview(p2){
  ...
  if (!penDrawFirst){
    if (typeof drawSnapIndicator==='function') drawSnapIndicator();
    if (typeof penDrawMouseCoordLabel === 'function') penDrawMouseCoordLabel(p2);  // Rev.19.31
    return;
  }
  ...
  if (!p2.lock && typeof drawSnapIndicator === 'function') drawSnapIndicator();
  // Rev.19.31: 마우스 옆 좌표 라벨
  if (typeof penDrawMouseCoordLabel === 'function') penDrawMouseCoordLabel(_p2);
  // 하단 상태바는 보조 안내만
  document.getElementById('statusHint').textContent =
...
function handlePenDrawClick(e){
  ...
  if (!penDrawFirst){
    ...
    redoStack=[]; pushHistory(); redrawDraw(); updateCount();
    document.getElementById('statusHint').textContent =
      `／ 텍스트선 시작 P${idx} · 두 번째 점 클릭 (Shift=직선/45°)` + (continuousMode?' · ⛓연속':'');
    return true;
  }
  // 2) 두 번째 점 — 선 생성
...
// ===== Rev.19.47: 기본 도형 입력 모달 (원/호·사각형·삼각형) =====
let penShapeEditId = -1;     // 수정 모드면 도형 id, 신규=-1
let penShapeCurType = 'arc';

// mm 좌표 → px 좌표 (직경좌표 모드면 그 좌표계 반영)
function penUserMmToPx(ux, uy){
  if (typeof penUserCoordToPx === 'function' && penOriginUserCoord){
    return penUserCoordToPx(ux, uy);
  }
  // 단순 모드: 원점(penOriginPx) 기준 + Y 뒤집기
  return { x: penOriginPx.x + ux/mmPerPixel, y: penOriginPx.y - uy/mmPerPixel };
}
// px → mm (역변환)
function penPxToUserMm(px, py){
  if (penOriginUserCoord){
    // 직경좌표 모드: 표시X = Cx − 2×dxMm
    const Cx = penOriginUserCoord.x || 0, Cy = penOriginUserCoord.y || 0;
    const dxMm = (px - penOriginPx.x) * mmPerPixel;
    const dyMm = -(py - penOriginPx.y) * mmPerPixel;
    return { x: Cx - 2*dxMm, y: Cy + dyMm };
  }
  return { x: (px - penOriginPx.x) * mmPerPixel, y: -(py - penOriginPx.y) * mmPerPixel };
}
// 길이(mm) → px (스케일만, 위치 변환 없음). 직경모드면 ×2 비율은 안 적용 (반지름은 절댓값)
function penMmLenToPx(mm){ return mm / mmPerPixel; }

function openPenShapeModal(type, existingShape){
  type = type || 'arc';
  penShapeCurType = type;
  penShapeEditId = existingShape ? existingShape.id : -1;
  const modal = document.getElementById('penShapeModal');
  if (!modal) return;
  modal.style.display = 'block';
  // 탭 활성화
  document.querySelectorAll('#penShapeTabs .ps-tab').forEach(b => {
    b.classList.toggle('active', b.dataset.type === type);
    b.onclick = () => { if (penShapeEditId < 0) { penShapeCurType = b.dataset.type;
      document.querySelectorAll('#penShapeTabs .ps-tab').forEach(x => x.classList.toggle('active', x.dataset.type === b.dataset.type));
      renderPenShapeFields(b.dataset.type, null); } };
  });
  // 수정 모드면 탭 고정
  if (existingShape){
    document.querySelectorAll('#penShapeTabs .ps-tab').forEach(b => { b.disabled = true; b.style.opacity = '0.5'; });
  } else {
    document.querySelectorAll('#penShapeTabs .ps-tab').forEach(b => { b.disabled = false; b.style.opacity = '1'; });
  }
  renderPenShapeFields(type, existingShape);
}

function closePenShapeModal(){
  const m = document.getElementById('penShapeModal');
  if (m) m.style.display = 'none';
  penShapeEditId = -1;
}
window.closePenShapeModal = closePenShapeModal;

function renderPenShapeFields(type, ex){
  const div = document.getElementById('penShapeFields');
  const hint = document.getElementById('penShapeHint');
  const title = document.getElementById('penShapeModalTitle');
  if (!div) return;
  if (type === 'arc'){
    title.textContent = ex ? '⭕ 원/호 수정' : '⭕ 원/호 (신규)';
    let cx=0, cy=0, dia=10, sd=0, ed=0;
    if (ex){
      const m = penPxToUserMm(ex.cx, ex.cy);
      cx = m.x; cy = m.y;
      dia = ex.r * 2 * mmPerPixel;
      // canvasRad = -userDeg*π/180  →  userDeg = -canvasRad*180/π
      if (ex.startAngle != null) sd = -ex.startAngle * 180 / Math.PI;
      if (ex.endAngle != null)   ed = -ex.endAngle * 180 / Math.PI;
    } else if (penOriginUserCoord){
      cx = penOriginUserCoord.x; cy = penOriginUserCoord.y;
    }
    div.innerHTML = `
      <label>중심 X (mm) <input id="psf-cx" type="text" value="${cx.toFixed(2)}"></label>
      <label>중심 Y (mm) <input id="psf-cy" type="text" value="${cy.toFixed(2)}"></label>
      <label>직경 (mm) <input id="psf-d" type="text" value="${dia.toFixed(2)}"></label>
      <label>시작 각도 (°) <input id="psf-sd" type="text" value="${sd.toFixed(1)}"></label>
      <label>끝 각도 (°) <input id="psf-ed" type="text" value="${ed.toFixed(1)}"></label>`;
    ...
    const selStyle = 'background:#1f1f1f; color:#fff; border:1px solid #555; border-radius:3px; padding:4px 6px; font-size:12px; width:140px;';
    div.innerHTML = `
      <label>기준점 위치
        <select id="psf-anchor" style="${selStyle}">
          <option value="tl"${anchor==='tl'?' selected':''}>┌ 좌상단</option>
          <option value="tr"${anchor==='tr'?' selected':''}>┐ 우상단</option>
          <option value="bl"${anchor==='bl'?' selected':''}>└ 좌하단</option>
          <option value="br"${anchor==='br'?' selected':''}>┘ 우하단 (베이스)</option>
          <option value="c"${anchor==='c' ?' selected':''}>＋ 중심</option>
        </select>
      </label>
      <label>기준점 X (mm) <input id="psf-x" type="text" value="${x.toFixed(2)}"></label>
      <label>기준점 Y (mm) <input id="psf-y" type="text" value="${y.toFixed(2)}"></label>
      <label>너비 W (mm) <input id="psf-w" type="text" value="${w.toFixed(2)}"></label>
      <label>높이 H (mm) <input id="psf-h" type="text" value="${h.toFixed(2)}"></label>`;
    hint.textContent = '기준점 위치 = 입력한 X,Y가 사각형의 어느 점인지 · 베이스용은 ┘ 우하단(직경축에 닿는 점) 사용 · Y는 위쪽 양수';
  } else if (type === 'triangle'){
    title.textContent = ex ? '△ 삼각형 수정' : '△ 삼각형 (신규)';
    let cx=0, cy=0, side=10, rot=0;
    if (ex && Array.isArray(ex.points) && ex.points.length>=3){
      // 무게중심·변길이·회전 추출
      const ps = ex.points.slice(0,3).map(p => penPxToUserMm(p.x, p.y));
      cx = (ps[0].x + ps[1].x + ps[2].x)/3;
      cy = (ps[0].y + ps[1].y + ps[2].y)/3;
      side = Math.hypot(ps[1].x-ps[0].x, ps[1].y-ps[0].y);
      // 첫 꼭짓점 방향 (중심→p0)
      rot = Math.atan2(ps[0].y - cy, ps[0].x - cx) * 180/Math.PI;
    } else if (penOriginUserCoord){
      cx = penOriginUserCoord.x; cy = penOriginUserCoord.y;
    }
    div.innerHTML = `
      <label>중심 X (mm) <input id="psf-cx" type="text" value="${cx.toFixed(2)}"></label>
      <label>중심 Y (mm) <input id="psf-cy" type="text" value="${cy.toFixed(2)}"></label>
      <label>한 변 길이 (mm) <input id="psf-side" type="text" value="${side.toFixed(2)}"></label>
      <label>회전 (°) <input id="psf-rot" type="text" value="${rot.toFixed(1)}"></label>`;
    hint.textContent = '정삼각형 · 중심 기준 / 회전 0°=첫 꼭짓점이 오른쪽, 90°=위 · 반시계';
  }
}

function _parseNum(id){ const v = parseFloat((document.getElementById(id)||{}).value); return isFinite(v) ? v : 0; }

function applyPenShape(){
  const type = penShapeCurType;
  const sw = parseInt((document.getElementById('strokeWidth')||{}).value) || 1;
  const stroke = (document.getElementById('strokeColor')||{}).value || '#ffffff';
  const layer = (currentLayer||'default');
  let newShape = null;
  if (type === 'arc'){
    const cx = _parseNum('psf-cx'), cy = _parseNum('psf-cy');
    const dia = _parseNum('psf-d'), sd = _parseNum('psf-sd'), ed = _parseNum('psf-ed');
    if (dia <= 0){ alert('직경은 0보다 커야 합니다'); return; }
    const cp = penUserMmToPx(cx, cy);
    const rPx = penMmLenToPx(dia/2);
    const isFull = (Math.abs(sd - ed) < 1e-3) || (Math.abs(Math.abs(sd-ed) - 360) < 1e-3);
    if (isFull){
      newShape = { id:++shapeIdSeq, type:'circle',
        p1:{x:cp.x, y:cp.y}, p2:{x:cp.x + rPx, y:cp.y},
        stroke, strokeWidth:sw, layer };
    } else {
      newShape = { id:++shapeIdSeq, type:'arc',
        cx:cp.x, cy:cp.y, r:rPx,
    ...
    const cx = _parseNum('psf-cx'), cy = _parseNum('psf-cy');
    const side = _parseNum('psf-side'), rot = _parseNum('psf-rot');
    if (side <= 0){ alert('변 길이는 0보다 커야 합니다'); return; }
    // 외접원 반지름 = side / √3
    const R = side / Math.sqrt(3);
    const cp = penUserMmToPx(cx, cy);
    const Rpx = penMmLenToPx(R);
    const pts = [];
    for (let k=0;k<3;k++){
      const deg = rot + k*120;
      const rad = -deg * Math.PI / 180;  // Y down 변환
      pts.push({ x: cp.x + Rpx*Math.cos(rad), y: cp.y + Rpx*Math.sin(rad) });
    }
    newShape = { id:++shapeIdSeq, type:'polyline',
      points: pts, closed:true,
      stroke, strokeWidth:sw, layer };
  }
  if (!newShape){ closePenShapeModal(); return; }
  if (penShapeEditId > 0){
    // 수정: 기존 도형 교체
    const idx = shapes.findIndex(s => s.id === penShapeEditId);
    if (idx >= 0){
      newShape.id = penShapeEditId;
      shapes[idx] = newShape;
    } else {
  ...
  redoStack = []; pushHistory(); redrawDraw(); updateCount();
  cmdLog(`◆ ${type} ${penShapeEditId>0?'수정':'추가'}`, 'user');
  document.getElementById('statusHint').textContent =
    `◆ ${type==='arc'?'원/호':type==='rect'?'사각형':'삼각형'} ${penShapeEditId>0?'수정 적용':'추가됨'}`;
  closePenShapeModal();
}
window.applyPenShape = applyPenShape;

// 도형 hit 테스트 (더블클릭 수정용) — 점은 제외, 기본도형만
function penFindShapeAt(p){
  const tolPx = 8 / (zoom||1);
  for (let i = shapes.length-1; i >= 0; i--){
    const s = shapes[i];
    if (s.type === 'circle' && s.p1 && s.p2){
      const r = Math.hypot(s.p2.x-s.p1.x, s.p2.y-s.p1.y);
      const d = Math.hypot(p.x-s.p1.x, p.y-s.p1.y);
      if (Math.abs(d - r) < tolPx) return s;
    } else if (s.type === 'arc' && s.cx != null){
      const d = Math.hypot(p.x-s.cx, p.y-s.cy);
      if (Math.abs(d - s.r) < tolPx) return s;
    } else if (s.type === 'rect' && s.p1 && s.p2){
      const xmin = Math.min(s.p1.x, s.p2.x), xmax = Math.max(s.p1.x, s.p2.x);
      const ymin = Math.min(s.p1.y, s.p2.y), ymax = Math.max(s.p1.y, s.p2.y);
      // 경계 위에 있는지
      const onLR = (Math.abs(p.x-xmin)<tolPx || Math.abs(p.x-xmax)<tolPx) && p.y>=ymin-tolPx && p.y<=ymax+tolPx;
      const onTB = (Math.abs(p.y-ymin)<tolPx || Math.abs(p.y-ymax)<tolPx) && p.x>=xmin-tolPx && p.x<=xmax+tolPx;
      if (onLR || onTB) return s;
    } else if (s.type === 'polyline' && Array.isArray(s.points) && s.points.length >= 3){
      // 삼각형으로 간주 — 변 위 hit
      for (let k=0; k<s.points.length; k++){
        const a = s.points[k], b = s.points[(k+1)%s.points.length];
        if (!s.closed && k === s.points.length-1) break;
        const dx = b.x-a.x, dy = b.y-a.y, len = Math.hypot(dx,dy);
        if (len < 1e-6) continue;
        const t = Math.max(0, Math.min(1, ((p.x-a.x)*dx + (p.y-a.y)*dy)/(len*len)));
        const px = a.x + t*dx, py = a.y + t*dy;
        if (Math.hypot(p.x-px, p.y-py) < tolPx) return s;
      }
    }
  }
  return null;
...
// 점 라벨(번호+좌표) 텍스트 재구성
function penRefreshLabels(){
  if (typeof shapes === 'undefined') return;
  shapes.forEach(s => {
    if (s.type === 'text' && s.penLabel !== undefined && s.penLabel >= 0){
      const idx = s.penLabel;
      const pt = penPoints[idx]; if (!pt) return;
      const num = 'P' + idx;
...
// 마우스 옆에 따라다니는 좌표 박스 (텍스트모드 mousemove에서 호출)
function penDrawMouseCoordLabel(p){
  if (!penOriginPx) return;
  const Z = zoom || 1;
  const lbl = penFormatUserCoord(p.x, p.y);
  preCtx.save();
  preCtx.font = `${12/Z}px monospace`;
  const padX = 5/Z, padY = 3/Z, lh = 14/Z;
  const w = preCtx.measureText(lbl).width;
  const boxW = w + padX*2, boxH = lh + padY*2;
  // 마우스 우하단에 (커서와 살짝 간격)
  let bx = p.x + 14/Z, by = p.y + 14/Z;
  if (bx + boxW > baseW) bx = p.x - boxW - 14/Z;
  if (by + boxH > baseH) by = p.y - boxH - 14/Z;
  if (bx < 0) bx = 4/Z;
  if (by < 0) by = 4/Z;
  preCtx.fillStyle = 'rgba(20,28,30,0.82)';
  preCtx.strokeStyle = '#7fb3d5';
  preCtx.lineWidth = 1/Z;
  if (preCtx.roundRect){ preCtx.beginPath(); preCtx.roundRect(bx, by, boxW, boxH, 3/Z); preCtx.fill(); preCtx.stroke(); }
  else { preCtx.fillRect(bx, by, boxW, boxH); preCtx.strokeRect(bx, by, boxW, boxH); }
  preCtx.fillStyle = '#7fb3d5';
  preCtx.textBaseline = 'top';
  preCtx.fillText(lbl, bx + padX, by + padY);
  preCtx.restore();
}
...
function penMovePoint(idx, newX, newY){
  const oldPt = penPoints[idx]; if (!oldPt) return;
  const dx = newX - oldPt.x, dy = newY - oldPt.y;
  if (Math.abs(dx) < 1e-9 && Math.abs(dy) < 1e-9) return;
  const ox = oldPt.x, oy = oldPt.y;
  ...
  shapes.forEach(s => {
    ...
    if (s.type === 'line'){
      ...
      if (s.p2 && Math.abs(s.p2.x - ox) < 1e-6 && Math.abs(s.p2.y - oy) < 1e-6){ s.p2.x = newX; s.p2.y = newY; }
    }
  });
  if (penShowCoordLabel) penRefreshLabels();
  redoStack = []; pushHistory(); redrawDraw(); updateCount();
  cmdLog(`🖐 P${idx} 이동 → ${penFormatUserCoord(newX, newY)}`, 'user');
}
...
function penToggleDiameterMode(){
  ...
  if (penOriginUserCoord){
    ...
    const cxEl = document.getElementById('penOriginCx');
    const cyEl = document.getElementById('penOriginCy');
  ...
  if (penShowCoordLabel) penRefreshLabels();
  // 토글 버튼 active 표시
  const b = document.getElementById('penBtnDiameter');
  if (b) b.classList.toggle('active', !!penOriginUserCoord);
}

// 점 좌표 라벨 토글
function penToggleCoordLabel(){
  penShowCoordLabel = !penShowCoordLabel;
  penRefreshLabels();
  const b = document.getElementById('penBtnCoordLabel');
  if (b) b.classList.toggle('active', penShowCoordLabel);
  cmdLog(`📍 점 좌표라벨 ${penShowCoordLabel?'ON':'OFF'}`, 'system');
}
...
function startTextMode(){
  ...
  if (penPickMode || (typeof penPoints !== 'undefined' && penPoints.length)){
    ...
    if (hasPts){
      ...
      document.getElementById('statusHint').textContent = `⌨ 텍스트 모드 (점 ${penPoints.length}개) · 마우스로 선긋기 가능(빈곳 클릭→끝점 클릭) · Shift=직선/45° · ⛓연속선토글`;
      if (typeof redrawDraw === 'function') redrawDraw();
      return;
    }
  }
...
(function(){
  ...
  if (bDia)   bDia.addEventListener('click',   penToggleDiameterMode);
  // 원점값 입력란 변경: 직경모드 ON이면 실시간 반영
  ['penOriginCx','penOriginCy'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.addEventListener('input', () => {
      if (penOriginUserCoord){
        penOriginUserCoord = {
          x: parseFloat(document.getElementById('penOriginCx').value)||0,
          y: parseFloat(document.getElementById('penOriginCy').value)||0
        };
        if (penShowCoordLabel) penRefreshLabels();
      }
    });
  ...
  if (typeof drawCanvas !== 'undefined' && drawCanvas){
    drawCanvas.addEventListener('dblclick', e => {
      ...
      // Rev.19.47: 점이 아니면 도형 hit → 기본도형 수정 모달
      const sh = (typeof penFindShapeAt === 'function') ? penFindShapeAt(p) : null;
      if (sh){
        e.preventDefault();
        let t = sh.type;
        if (t === 'circle') t = 'arc';  // 완전원도 arc 모달
        else if (t === 'polyline') t = 'triangle';
        if (t === 'arc' || t === 'rect' || t === 'triangle'){
          openPenShapeModal(t, sh);
        }
      }
    });
  }
...
(function(){
  ...
  const CMD_CATS = {
    ...
    '기본도형': [
      ...
      { l:'⭕ 원/호', action: () => {
          if (typeof penPickMode !== 'undefined' && !penPickMode && typeof startTextMode === 'function') startTextMode();
          if (typeof openPenShapeModal === 'function') openPenShapeModal('arc', null);
        }, ex:'⭕ 원/호: 중심X,Y · 직경 · 시작°~끝° (반시계, 시작=끝이면 완전 원) · 적용 → 도형 추가' },
      { l:'▭ 사각형', action: () => {
          if (typeof penPickMode !== 'undefined' && !penPickMode && typeof startTextMode === 'function') startTextMode();
          if (typeof openPenShapeModal === 'function') openPenShapeModal('rect', null);
        }, ex:'▭ 사각형: 좌상단X,Y · W,H 입력 → 도형 추가' },
      { l:'△ 삼각형', action: () => {
          if (typeof penPickMode !== 'undefined' && !penPickMode && typeof startTextMode === 'function') startTextMode();
