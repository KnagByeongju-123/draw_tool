// ##### draw_tool_core2.js  Rev.17.9  최신본 — 명령어 (마우스원점지정·절교/절각[수평/수직]·점·선·지름·거리두기·연장·기준점·방향교점·각교점·호·=수식·이동) #####
...
function tryPenCommand(cmdStr){
  ...
  if (toks[0] === '두께'
    ...
    if (isBoth){
      const half = dmm/2/mmPerPixel;
      const nL = buildSide(true,  half);
      const nR = buildSide(false, half);
      penFinish(`▦ ${startI}~${endI} 경로 두께 ${dmm}mm 양쪽 평행 (좌${nL}+우${nR})`);
    } else {
